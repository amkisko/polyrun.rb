...
# CI: RSpec JSON → JUnit XML (single-process runs). When Polyrun sets POLYRUN_SHARD_INDEX
# (parallel-rspec workers), emit JSON only to tmp/rspec-<shard>.json so coverage can run
# report-junit on tmp/rspec-*.json.
if ENV["CI"]
