...
This RFC defines how Polyrun design changes are proposed, written, numbered, and advanced.

## Motivation

Shard layout, coverage fragment files, `POLYRUN_SHARD_*` environment, hook lifecycle, and `polyrun.yml` partition keys are the public contract. Local `run-shards` and CI `ci-shard-run` are different layouts; mixing them silently breaks coverage gates. Numbered RFCs are the unit for a change that would move the gem version for every host CI file. RFC 0002 records those problems and the shipped commands.
