...
- **No subcommand**: from the app root, `polyrun` (or `./bin/polyrun`) with **no** subcommand runs parallel tests using the resolved suite: `partition.suite` when set (`rspec` / `minitest` / `quick`); else suite inferred from `partition.paths_file` when that file lists `_spec.rb` / `_test.rb` / other Quick `.rb` files; else filesystem auto-detect (RSpec under `spec/` before Minitest under `test/`, then Polyrun Quick). Explicit subcommands: `parallel-rspec`, `parallel-minitest`, `parallel-quick`, or `start`.
...
- **`partition.suite`**: in `polyrun.yml`, set `partition.suite` to `auto` (default), `rspec`, `minitest`, or `quick`. For bare `polyrun` / default dispatch it selects the runner. For `run-shards` path globs it selects which tree to list when there is no `paths_file`. Explicit `minitest` / `quick` skips legacy `spec/spec_paths.txt`. When `paths_file` is set, its contents must match an explicit suite (else exit 2). `run-shards` also exits 2 if the command after `--` looks like a different suite than the path list (e.g. Minitest paths with `bundle exec rspec`, or RSpec paths with `bin/rails test`). Mixed `_spec.rb` / `_test.rb` lists are allowed only with a suite-agnostic custom command. `parallel-*` commands do not run `start` prepare or database bootstrap.
