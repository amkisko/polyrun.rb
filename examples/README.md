...
- **`partition.suite`**: in `polyrun.yml`, set `partition.suite` to `auto` (default), `rspec`, `minitest`, or `quick`. For bare `polyrun` / default dispatch it selects the runner. For `run-shards` path globs it selects which tree to list when there is no `paths_file`. When `paths_file` is set, its contents must match an explicit suite (else exit 2). `run-shards` also exits 2 if the command after `--` looks like a different suite than the path list (e.g. Minitest paths with `bundle exec rspec`).
